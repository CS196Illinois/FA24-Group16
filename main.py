...
def player_bet(player_balance, current_bet):
    if current_bet == 0:
        decision = input(f"Your move (check, bet, fold). Your balance: ${player_balance}: ").strip().lower()
        while decision not in ['check', 'bet', 'fold']:
            decision = input("Invalid move. Choose 'check', 'bet', or 'fold': ").strip().lower()
    else:
        decision = input(f"Your move (call, raise, fold). Current bet: ${current_bet}, Your balance: ${player_balance}: ").strip().lower()
...
def texas_holdem():
    ...
    print(f"\nCommunity cards after flop: {community_cards}")
